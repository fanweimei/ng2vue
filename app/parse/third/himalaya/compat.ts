...
export function endsWith(str, searchString, position = 0) {
  const index = (position || str.length) - searchString.length;
  const lastIndex = str.lastIndexOf(searchString, index);
  return lastIndex !== -1 && lastIndex === index;
}

export function stringIncludes(str, searchString, position = 0) {
...
export function arrayIncludes(array, searchElement, position = 0) {
