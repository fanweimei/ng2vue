  ...
  return node.parameters.reduce(
    (all: TshParameter[], cur: ParameterDeclaration) => {
      ...
      if (isIdentifier(cur.name)) {
        params.push(
          new TshParameter(
            (cur.name as Identifier).text,
            getNodeType(cur.type),
            cur.getStart(),
            cur.getEnd()
          )
        );
        ...
        const boundParam = new ObjectBoundParameterDeclaration(
          cur.getStart(),
          cur.getEnd()
        );
        ...
        if (cur.type && isTypeReferenceNode(cur.type)) {
          ...
          types = cur.type.members
            ...
            .map((signature: any) =>
              getNodeType((signature as PropertySignature).type)
            );
        }
        ...
        const elements = cur.name.elements;
        let types: (string | undefined)[] = [];
        const boundParam = new ArrayBoundParameterDeclaration(
          cur.getStart(),
          cur.getEnd()
        );
...
export function parseFunction(
  ...
  const name = node.name
    ? node.name.text
    : getDefaultResourceIdentifier(resource);
